...
Reads the raw dataset and produces two sets of train/test splits:
  - Spatial split  (merged_s_train.csv / merged_s_test.csv):
      80 % / 20 % random split by administrative unit (uid).
  - Temporal split (merged_t_train.csv / merged_t_test.csv):
...
def preprocess(
    ...
    s_train_path = os.path.join(spatial_dir, "merged_s_train.csv")
    s_test_path  = os.path.join(spatial_dir, "merged_s_test.csv")
    ...
    t_train_path = os.path.join(temporal_dir, "merged_t_train.csv")
    t_test_path  = os.path.join(temporal_dir, "merged_t_test.csv")
...
if __name__ == "__main__":
    ...
    SPATIAL_DIR  = config.DATA_DIR
    TEMPORAL_DIR = config.DATA_DIR
